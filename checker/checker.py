...
def white_diff_step(output_filename, correct_output_filename):
    ...
    if os.path.isfile(output_filename):
        out_file = open(output_filename, mode='r')
        res_file = open(correct_output_filename, mode='r')
        return white_diff_fobj_step(out_file, res_file)
    else:
        return 0.0, [
            "Evaluation didn't produce file %s", output_filename]

if __name__ == '__main__':
    res = white_diff_step(sys.argv[3], sys.argv[2])
    print(res[1][0], file=sys.stderr)
    print(res[0])
